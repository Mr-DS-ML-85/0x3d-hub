...
MARKcan be used to display comments in the column
...
TODOis used to display reminders of things that need to be done
...
FIXMEis used to display reminders about things that need to be fixed

## Variable

### Variable declaration

Variables are declared withvar:
...
Constants are declared withlet:
...
In the example below, distanceInFeet has agetterand asetter. Because of thesetter, thegetterrequires the
keywordget:
...
The new value can be accessed inwillSet:
...
willSetcan be used to execute some code before setting the variable value
...
-<less than->greater than-<=less than or equal to->=greater than or equal to-==is
...
-a...bClosed Range-a..<bHalf-Open Range-...bOne-Sided Range
...
!false && truefirst evaluates and returnstrueThen, the expression,true||falseevaluates and returns the
final resulttrue
...
true && falsefirst evaluates to returnfalseThen, the expression,false||falseevaluates and returns the
final resultfalse
...
Thecontinuekeyword will force the loop to continue for the next iteration
...
Awhileloop accepts a condition and keeps executing its body code while the provided condition istrue. If the
condition is never false, the loop will keep running and the program will get stuck in aninfinite loop
...
We can use a collection (Set) to storeuniqueelements of the same data type
...
To create a set filled with values, use theSetkeyword before the assignment operator.
...
Anunorderedcollection of pairs of data or key-value pairs
...
Eachkeyisuniqueeven if they all contain the samevalue
...
Contains onlyStringkeys andIntvalues
...
To change the value of a key-value pair, use the.updateValue()method or the subscript syntax by appending brackets[ ]with the existing keys within to the name of the dictionary, then adding the assignment operator(=)followed
...
the requested type, it should returnnil
...
Structs or structs are used to programmatically represent real-life objects in code. A structure is created using thestructkeyword, followed by its name, followed by a body containing its properties and methods
...
A class can be initialized using theinit()method and the corresponding initialization properties. In theinit()method, theselfkeyword is used to refer to the actual instance of the class assigning property values
...
SavingsAccountextendsBankAccountclass
...
The newSavingsAccountclass (subclass) automatically gets all the characteristics of theBankAccountclass
(superclass). Additionally, theSavingsAccountclass defines an.interestproperty and an.addInterest()method.
...
TheResolutionstructure definition and theVideoModeclass definition only describe the appearance ofResolutionorVideoMode, create an instance of the structure or class:
...
Add conformance to theCaseIterableprotocol to access theallCasesproperty, which returns an array of all cases of
the enumeration
...
enum, it needs to be markedmutating
...
let hello2 = Hello(rawValue: "ÐÑÐ¸Ð²ÐµÑ")
...
Generics enable us to write clear and concise code that works with any data type. By using placeholders (likeT), this
...
The placeholdersTis an example of a type parameter, are written inside angle brackets(such as<T>).

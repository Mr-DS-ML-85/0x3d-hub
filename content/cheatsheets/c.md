...
In%-9d,dmeans to output in10base,9means to occupy at least9characters width, and the width is not
...
Cdoes nothave a String type, usechartype and create anarrayof characters
...
Break out of the loop wheniis equal to4
...
| --- | --- | --- | --- |
| `char` | 1 byte | `â128` ~ `127` | single character/alphanumeric/ASCII |
| `signed char` | 1 byte | `â128` ~ `127` |  |
| `unsigned char` | 1 byte | `0` ~ `255` |  |
| `int` | `2` to `4` bytes | `â32,768` ~ `32,767` | store integers |
| `signed int` | 2 bytes | `â32,768` ~ `32,767` |  |
| `unsigned int` | 2 bytes | `0` ~ `65,535` |  |
| `short int` | 2 bytes | `â32,768` ~ `32,767` |  |
| `signed short int` | 2 bytes | `â32,768` ~ `32,767` |  |
...
ANSI Cdefines a number of macros that you can use, but you cannot directly modify these predefined macros
...
- cbrt(x)cube root
- cos(x)cosine
- the value ofexp(x)Ex
- sin(x)the sine of x
- tangent oftan(x)angle
...
Create a struct variable calleds1
...
Assigning values to strings using thestrcpyfunction
...
int main() {
  ...
  struct myStructure s1;
  // Assign values ââto the members of s1
  s1.myNum = 13;
  s1.myLetter = 'B';
...
// Assign values ââto different structure variables
s1.myNum = 13;
s1.myLetter = 'B';
...
In the example, the value ofs1is copied tos2
...
There are many functions in the C library toopen/read/write/searchandclosefiles
...
After performing all operations on the file, the file must be closed withfclose()

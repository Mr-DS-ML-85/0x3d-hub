...
- Prompt: input text that guides an AI modelâs response.
...
- Restructure text to eliminate filler words that donât add meaning:

```{.wrap}
Example: âprovide a breakdown" -> "provide sales breakdownâ.
...
```{.wrap}
Example: âFilter this piece of text" -> "Filter textâ
...
```{.wrap}
Example: âDo you think our site is good" -> "Explain flaws in the designs of each webpageâ
...
```{.wrap}
Example: âexplain our sales" -> "explain a breakdown of our sales performance for the last quarterâ.
...
- Explain what X does
- Explain X [in simple terms | like Iâm 5]; Simplify X
...
```{.wrap}
Suggest creative ideas for 10 year oldâs birthday

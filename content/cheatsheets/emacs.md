...
| `C-x` `<` | Scroll left |
| `C-x` `>` | Scroll right |
| `C-l` | Scroll current line tocenter, top, bottom |
...
The help system is simple. TypeC-h(orF1) and follow the directions. If you are a first-time user, typeC-htfor a tutorial.
...
| `n` | Select nth menu item by number (1â9) |
...
TypeC-xESCESCto edit and repeat the last command that used the minibuffer. TypeF10to activate menu bar
...
UseC-sorC-ragain to repeat the search in either direction. If Emacs is still searching,C-gcancels only the
part not matched.
...
```{.wrap}
(global-set-key (kbd "C-c g") âsearch-forward)
(global-set-key (kbd "M-#") âquery-replace-regexp)
...
The interactive spec says how to read arguments interactively. TypeC-hfinteractiveRETfor more details.

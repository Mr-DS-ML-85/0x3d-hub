...
- GemfileIs the Bundler (also gem) configuration file that contains the
...
checkUser Guidecap 19,20,21,22,23Pre-Defined Variables and Constants
...
| `Integer` | a = 17 | a.class > Integera.class.superclass > Numeric | # |
| `Float` | a = 87.23 | a.class > Floata.class.superclass > Numeric | # |
| `String` | a = "Hello universe" | a.class > String | # |
| `Array` | a = [12, 34] | a.class > Array | # |
| `Hash` | a = {type: "tea", count: 10} | a.class > Hash | # |
| `Boolean` | a = falsea = true | a.class > FalseClassa.class > TrueClass | TrueClassFalseClass |
...
when 19
  puts "It's 19"
  # æ£æ¥ä¸ä¸ªç¡®åçå­ç¬¦ä¸²ï¼âZamanâ
when "Zaman"
  puts "Hi Zaman"
when 7..11
...
| strip | `" hEllo WOrlD ".strip` `"hEllo WOrlD"` | It will remove (trim) any leading and trailing characters: null (â\x00â), horizontal tab (â\tâ), newline (\n), vertical tab (â\vâ), form feed (f), carriage return(\r), space (" ") |
...
In ruby, methods ending with a question mark (?) are called boolean methods, which returntrueorfalse
...
The code betweendoandend(for multiple lines) or curly braces{and}(for a single line) is called a block,
...
arr = ['a', 'b', 'c', 'd', 'e', 'f']
arr.fetch(100)
#=> IndexError: Index outside array bounds 100ï¼-6...6
...
### Check if a value exists in an arrayï¼includeï¼ï¼
...
When you have a multi-line block, you can replace the curly braces{}withdoandend
...
to compare items
- intersectionReturns a new array containing each element found in self and all given arrays other_arrays; duplicates
...
- unionReturns an array containing all elements of self and all elements of the given array, with duplicates removed
- differenceReturns an array containing all elements of self not found in any given array
- productself Returns or produces all combinations of elements from self and the given array

...
The/means quotient of x and y, and the//means floored quotient of x and y, also seeStackOverflow
...
len(tuple) â Returns the number of elements.
in â Checks if an element exists in the tuple.
Concatenation (+) â Combines two tuples.
Repetition (*) â Repeats a tuple.
Slicing (tuple[start:end]) â Extracts a subtuple.
